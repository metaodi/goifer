...
for faction in factions:
    ...
    members = client.search('Mitglieder', query=f"dauer_end >= \"2020-11-19 00:00:00\" and dauer_start <= \"2020-11-19 00:00:00\" and gremium adj {faction['kurzname']} sortBy name/sort.ascending vorname/sort.ascending")
    contacts = [m['person']['kontakt'] for m in members]
    name_members = [f"{' '.join([m['name'], m['vorname'] or '']).strip()} ({m['ort']})" for m in contacts]
    dump(name_members)
